...
public:
  double
  trig_fcn (double x) const override
  {
    return std::sin (x);
  }
  ...
  // If you implement a destructor on the Derived class, use the override
  // keyword. Use this for Case 2 noted in the Base class comments:
  //~Derived() override { std::cout << "Derived destructor (override) called\n";
  //}

  // Case 3 (see Base class comments): It usually suffices to just use a virtual
  // default destructor on the base class, without implementing a destructor on
  // derived classes (In this case, comment out the all of the destructor
  // implementations and use virtual ~Base() = default; in the Base class above.
};
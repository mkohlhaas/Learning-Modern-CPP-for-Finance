...
double
BlackScholes::operator() (double vol) const
{
  using std::exp;
  ...
  if (time_to_exp_ > 0.0)                          // (2)
    ...
    {
      return std::max (phi * (spot_ - strike_),
                       0.0); // (8) (std::max in <algorithm>)
    }
}

std::map<RiskValues, double>
BlackScholes::risk_values (double vol)
{
  ...
  double gamma = exp (-div_ * time_to_exp_) * norm_pdf (d1) / (spot_ * vol * sqrt (time_to_exp_));
  ...
  double theta = phi * div_ * spot_ * exp (-div_ * time_to_exp_) * nd_1
                 ...
                 - spot_ * exp (-div_ * time_to_exp_) * norm_pdf (d1) * vol / (2.0 * sqrt (time_to_exp_));
...
std::array<double, 2>
BlackScholes::compute_norm_args_ (double vol) const
{
...
double
BlackScholes::norm_cdf_ (double x) const
{
  return (1.0 + std::erf (x / std::numbers::sqrt2)) / 2.0;
}

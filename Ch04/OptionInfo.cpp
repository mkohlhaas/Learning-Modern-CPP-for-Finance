...
double
OptionInfo03::option_payoff (double spot) const
{
  return payoff_ptr_->payoff (spot);
}

double
OptionInfo03::time_to_expiration () const
{
  return time_to_exp_;
}

void
OptionInfo03::swap (OptionInfo03 &rhs)
{
  using std::swap;
...
OptionInfo03::~OptionInfo03 () { delete payoff_ptr_; }
...
// of a raw pointer and "virtual constructor" as seen in the C++03 version
// above.
...
double
OptionInfo::option_payoff (double spot) const
{
  return payoff_ptr_->payoff (spot);
}

double
OptionInfo::time_to_expiration () const
{
  return time_to_exp_;
}

void
OptionInfo::swap (OptionInfo &rhs) noexcept
{
  using std::swap;
  swap (payoff_ptr_, rhs.payoff_ptr_);
  swap (time_to_exp_, rhs.time_to_exp_);
  // throw std::runtime_error{"Error:..."};	// Compiler warning, runtime
...
cpp03_examples ()
{
  ...
  cout << "\n"
       << format ("Payoff for call option with {} year(s) to expiration = {}", call_opt.time_to_expiration (),
                  call_opt.option_payoff (85.0))
       << "\n";
  ...
  cout << "\n"
       << format ("Payoff for call option (copy ctor) with {} year(s) to "
                  "expiration = {}",
                  copy.time_to_expiration (), copy.option_payoff (90.0))
       << "\n";

  cout << "\n"
       << format ("Payoff for call option (copy assgn) with {} year(s) to "
                  "expiration = {}",
...
cpp11_examples ()
{
  ...
  // Alternatively, this could be written in one line with an anonymous
  // temporary unique pointer:
  ...
  // call_copy and call_copy_assgn; otherwise, comment out the following `#if 0`
  // block.
  // #if 0
  OptionInfo call_copy{ call_opt }; // Need copy operations implemented on OptionInfo
  cout << "\n"
       << format ("Payoff for _copied_ call option with {} year(s) to "
                  "expiration = {}",
  ...
  cout << "\n"
       << format ("Payoff for _moved_ (ctor) call option with {} year(s) to "
                  "expiration = {}",
  ...
  cout << "\n"
       << format ("Payoff for _moved_ (=) call option with {} year(s) to "
                  "expiration = {}",
...
/*
        A portion of this sample code (C++03 example of RAII) is based on sample
   code provided on CD to accompany the book _C++ Design Patterns and
   Derivatives Pricing (2E)_, by Mark Joshi, 2008.

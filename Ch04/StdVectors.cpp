...
void
std_vectors () // Top calling function
{
  push_back_example ();
...
{
public:
  ~MakeItClear () { std::cout << "MakeItClear destructor called" << "\n"; }
};
...
push_back_example ()
{
  ...
  cout << format ("Bank account vector contains {} elements, and capacity = {}.", ba_push.size (), ba_push.capacity ())
       << "\n\n"; // size = 0, capacity = 5
  ...
  for (const auto &ba : ba_push)
    {
      ...
      cout << "Accumulated amount after 1 year = " << std::setprecision (2) << std::fixed << ba.value (1.0) << "\n";
    }
...
reserve_and_capacity ()
{
  ...
  cout << format ("Size of vector (push) = {}, capacity = {}, remaining capacity = {}\n", ba_push.size (),
                  ba_push.capacity (), ba_push.capacity () - ba_push.size ());
  ...
  ba_emplace.emplace_back (2000.00, 0.022);
  ba_emplace.emplace_back (3000.00, 0.023);
  cout << format ("Size of vector (emplace) = {}, capacity = {}, remaining "
                  "capacity = {}\n\n",

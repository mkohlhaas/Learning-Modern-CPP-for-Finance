...
template <typename T> T tmpl_square (const T &t);

template <typename T>
T
tmpl_square (const T &t)
{
  return t * t;
}
...
template <typename T> MyPair<T>::MyPair (T first, T second) : a_ (first), b_ (second) {}

template <typename T>
T
MyPair<T>::get_min () const
{

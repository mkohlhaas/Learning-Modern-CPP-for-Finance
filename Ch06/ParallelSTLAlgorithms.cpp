...
parallel_min_max_mean ()
{
  ...
  auto next_norm = [&mt, &nd] () -> double { return nd (mt); };
  ...
  double mean_parallel = 1.0 / norms.size () * std::reduce (norms.begin (), norms.end (), 0.0);
...
parallel_dot_product ()
{
  ...
  int sum_diff = std::transform_reduce (v.begin (), v.end (), w.begin (), 0, std::plus<int>{}, std::minus<int>{});
...
void
exp_approx (unsigned num_elements, unsigned n)
{
  ...
  auto next_norm = [&mtre, &nd] (double x) { return nd (mtre); };
  ...
  // You need not be concerned with its implementation (not covered in the
  // book). It is just provided for the examples in the book, and/or if you wish
  // to use it elsewhere.
  Timer tmr{};

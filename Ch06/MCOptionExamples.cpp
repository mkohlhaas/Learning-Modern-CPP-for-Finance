...
void
mc_option_examples () // Top calling function
{
...
euro_no_barrier_examples ()
{
  ...
  int num_scenarios = 20'000; unsigned seed = 42;
...
euro_with_barrier_examples ()
{
  ...
  cout << format ("ITM Call option with time value = {}, dividend rate = {}, "
                  "up/out barr_val = {}):\n",
                  time_to_exp, div, barr_val);

  OptionInfo        opt_call_itm_barr{ std::make_unique<CallPayoff> (strike), time_to_exp };
  MCOptionValuation val_call_itm_barr{
    std::move (opt_call_itm_barr), num_time_steps, vol, rate, div, barr_type, barr_val
  };
  ...
  num_time_steps = 2'400; num_scenarios = 50'000;

      OptionInfo    opt_call_itm_barr_2{ std::make_unique<CallPayoff> (strike), time_to_exp };
  MCOptionValuation val_call_itm_barr_2{
    std::move (opt_call_itm_barr_2), num_time_steps, vol, rate, div, barr_type, barr_val
  };
  ...
  num_scenarios  = 20'000;
                  ...
                  cout
                  << format ("ITM Put option with time value = {}, dividend rate = {}, "
                             "up/out barr_val = {}):\n",
                             time_to_exp, div, barr_val);

  OptionInfo        vo_put_itm_not_exp{ std::make_unique<PutPayoff> (strike), time_to_exp };
  MCOptionValuation val_put_itm_not_exp{
    std::move (vo_put_itm_not_exp), num_time_steps, vol, rate, div, barr_type, barr_val
  };
  ...
  MCOptionValuation val_put_itm_not_exp_2{
    std::move (vo_put_itm_not_exp_2), num_time_steps, vol, rate, div, barr_type, barr_val
  };

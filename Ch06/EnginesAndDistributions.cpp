...
// These will be different than what one gets with the MSVC version, but both
// are fully compliant with the C++ Standard.
...
mersenne_twister_and_normal_dist ()
{
  ...
  // Generate 10 standard normal variates and set as elements in the vector
  // 'norms':
  for (double &x : norms)
    {
...
max_drawdown_sim ()
{
  ...
  vector<double> pnl{ -149 '299.30, -673' 165.13,
                      3 '891' 123.79,
                      1 '061' 346.21,
                      -578'464.00, - 260 '855.99, 1' 102 '167.76, 509' 764.96,
                      -276 '786.46,  -11' 947.13,
                      -277 '781.70, -318' 603.05,
                      57 '747.06,    151' 336.99,
                      267'826.14, - 198 '132.05, -175' 232.84,
                      -5 '973.61,    177' 224.51,
                      -711'878.76, - 276 '786.56, 116' 489.15,
                      -185 '188.30,  1' 183 '810.47, 1' 563'147.38, - 83 '632.98,  527' 687.09,
                      -307 '650.96,  -321' 589.76,
                      -1 '434' 711.00,
                      742 '743.96,  -245' 921.36,
                      -198 '131.26,  399' 250.02,
                      -311'632.90, 326 '569.83,  437' 084.67,
                      -297 '694.80,  -379' 336.81,
                      -173'240.27, - 62 '724.74,  -363' 407.14,
                      -142 '375.97,  -103' 546.19,
                      187'179.27, - 161 '293.14, -131' 423.78,
                      1 '195' 759.19,
                      198 '131.95,   -229' 991.59,
                      -109 '519.00, -148' 348.69,
                      1 '447' 621.95 };
  ...
  // double max_dd = 100.0 * (*std::ranges::max_element(drawdowns));
  // // For percentage form cout << std::fixed << std::setprecision(2) << "Max
  // DD from backtest = "
  ...
  // Results:
  cout << std::fixed << std::setprecision (2) << "Max DD from backtest = $" << max_dd << ", Net P/L = $"
       << cum_pnl.back () << ", Net Profit/MaxDD = " << net_pl_over_max_dd << "\n\n";
  ...
  // at the (1 - alpha) * drawdowns.size() position. (Check for rounding
  // function)

...
/*
MCOptionValuation::MCOptionValuation(OptionInfo&& opt, int time_steps, double
vol, double int_rate, double div_rate) : opt_{std::move(opt)},
time_steps_{time_steps}, vol_{vol}, int_rate_{int_rate}, div_rate_{div_rate} {}
*/

// This is the more general form of the constructor, which allows for barrier
// options:
MCOptionValuation::MCOptionValuation (OptionInfo &&opt, int time_steps, double vol, double int_rate, double div_rate,
                                      BarrierType barrier_type, double barrier_value)
    : opt_{ std::move (opt) }, time_steps_{ time_steps }, vol_{ vol }, int_rate_{ int_rate }, div_rate_{ div_rate },
...
double
MCOptionValuation::calc_price_euro (double spot, int num_scenarios, unsigned unif_start_seed)
{
  if (opt_.time_to_expiration () > 0.0) // (1)
    {
      ...
      for (int i = 0; i < num_scenarios; ++i)                                                                    // (4)
        {
          EquityPriceGenerator epg{ spot, time_steps_, opt_.time_to_expiration (), vol_, int_rate_, div_rate_ }; // (5)
          vector               scenario = epg (unif_int_dist (mt_unif)); // (unif_int_dist(mt_unif): next seed)

          discounted_payoffs.push_back (disc_factor * opt_.option_payoff (scenario.back ())); // (6)
        }

      return (1.0 / num_scenarios)
             * std::accumulate (discounted_payoffs.cbegin (), discounted_payoffs.cend (),
                                0.0); // (7)
    }
  else
    {
...
double
MCOptionValuation::calc_price (double spot, int num_scenarios, unsigned unif_start_seed)
{
  bool barrier_hit = (barrier_type_ == BarrierType::up_and_out && spot >= barrier_value_)
                     || (barrier_type_ == BarrierType::down_and_out && spot <= barrier_value_); // (1)

  if (barrier_hit)
    {
      return 0.0; // Option is worthless		// (2)
    }
  ...
  if (opt_.time_to_expiration () > 0)                                     // (3)
    {
      ...
      for (int i = 0; i < num_scenarios; ++i)                            // (6)
        {
          EquityPriceGenerator epg{ spot, time_steps_, opt_.time_to_expiration (), vol_, int_rate_, div_rate_ };
          ...
          switch (barrier_type_)                                         // (8)
            ...
            case BarrierType::up_and_out:                                // (10)
              {
                auto barrier_hit_pos = std::find_if (scenario.cbegin (), scenario.cend (), [this] (double sim_eq) {
            ...
            case BarrierType::down_and_out:                                                 // (13)
              {
                auto barrier_hit_pos = std::ranges::find_if (
                    scenario, [this] (double sim_eq) { return sim_eq <= barrier_value_; }); // (14)
                if (barrier_hit_pos != scenario.cend ())
                  {
                    barrier_hit = true;
                  }
...
double
MCOptionValuation::calc_price_par (double spot, int num_scenarios, unsigned unif_start_seed)
{
  bool barrier_hit = (barrier_type_ == BarrierType::up_and_out && spot >= barrier_value_)
  ...
  if (opt_.time_to_expiration () > 0)
    {
          ...
          for (int i = 0; i < num_scenarios; ++i)
            {
              ...
              // see "Concluding Remarks" in book.  You can comment out (2) and
              // replace with the following:
              // ftrs.push_back(std::async(std::launch::async, epg,
              // unif_int_dist(mt_unif)));
            }
          ...
          for (auto &ftr : ftrs)
            {
              ...
              switch (barrier_type_)
                {
                case BarrierType::none:
                  break; // do nothing, proceed to appending discounted terminal price
                ...
                case BarrierType::down_and_out:
                  {
                    auto barrier_hit_pos
                        = std::ranges::find_if (scenario, [this] (double sim_eq) { return sim_eq <= barrier_value_; });
                    if (barrier_hit_pos != scenario.cend ())
                      {
                        barrier_hit = true;
                      }
      ...
      return (1.0 / num_scenarios) * std::accumulate (discounted_payoffs.cbegin (), discounted_payoffs.cend (), 0.0);
    }
  else
    {

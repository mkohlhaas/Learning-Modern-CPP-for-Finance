...
void
concepts_examples () // Top level calling program
{
  add_stuff ();
  square_stuff ();
  quadratic_stuff ();
...
add_stuff ()
{
  ...
  vector<int> u{ 1, 2, 3 };
  vector<int> v{ 4, 5, 6 };
  // vector<int> sum_vec = add_them(u, v);		// Error: + not defined
  // (intentionally
...
void
quadratic_stuff () // Using pre-defined Standard Library concepts
{
  using std::cout;
  using std::format;
  using std::string;
  ...
  // FPQuadratic<int> fp_quad_int(2, 4, 2);		// Will not compile -
  // floating point enforced by concept.
  ...
  // Quadratic<SimpleClass> quad_simple{SimpleClass{0}, SimpleClass{1},
  // SimpleClass{2}};

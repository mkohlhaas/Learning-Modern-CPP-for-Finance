...
template <typename T> concept can_add = requires (const T &t1, const T &t2) { t1 + t2; };

template <typename T>
T
add_them (const T &t1, const T &t2) requires can_add<T>
{
  return t1 + t2;
}

// Add/Subtract:
template <typename T> concept can_add_and_subtr = requires (const T &t1, const T &t2)
{
  t1 + t2;
  t1 - t2;
};

template <typename T> concept can_subtract = requires (const T &t1, const T &t2) { t1 - t2; };

template <typename T> concept can_add_and_subtr_combined = requires (const T &t) { can_add<T> &&can_subtract<T>; };

template <typename T> concept can_square = requires (const T &t) { t *t; };
...
template <typename T> requires std::floating_point<T> class FPQuadratic
{
public:
...
template <typename T> concept Number = requires (T t) { std::integral<T> || std::floating_point<T>; };

// Can just use the Number concept as the template parameter:
template <Number N> class Quadratic
{
public:

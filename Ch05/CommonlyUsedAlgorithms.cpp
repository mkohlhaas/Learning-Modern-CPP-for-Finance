...
void
commonly_used_algorithms () // Top calling function
{
  for_each_examples ();
...
transform_examples_different_container ()
{
  ...
  std::transform (v.begin (), v.end (), std::back_inserter (dq), [] (int n) { return tmpl_square (n) + 0.5; });
  ...
  std::ranges::transform (v, std::back_inserter (dq2), [] (int n) { return tmpl_square (n) + 0.5; });
...
std_begin_and_std_end ()
{
  ...
  // we can replace this clunky syntax with a cleaner and more expressive
  // version:
  ...
  std::transform (std::begin (c_array_ints), std::end (c_array_ints), std::begin (c_array_ints),
...
member_fcn_as_aux_fcn ()
{
  ...
  auto quad = [&q] (double x) -> double { return q.value (x); };

...
first_example_count_if ()
{
  ...
  cout << "\nUsing std::count_if algorithm with is_odd(.) auxiliary "
          "(predicate) function -- results:\n";
  cout << format ("Number of odd elements = {}\n\n", num_odd);

  // Some variations on the theme:
  auto num_even = std::count_if (int_vec.begin (), int_vec.end (), [] (int n) { return !is_odd (n); });

  // Count the number of elements congruent modulo 3 to some remainder int
  // value:
  const int rem                 = 2;
  auto      num_congruent_mod_3 = std::count_if (int_vec.begin (), int_vec.end (), [] (int n) { return n % 3 == rem; });

  // count the number of elements in[3, 6]:
  auto num_within_interval
      = std::count_if (int_vec.begin (), int_vec.end (), [low = 3, high = 6] (int n) { return low <= n && n <= high; });

  cout << format ("num_even = {}, num_congruent_mod_3 = {}, num_within_interval "
                  "[3, 6] (closed interval) = {}\n\n",
...
first_ex_with_other_containers ()
{
  ...
  auto is_odd_value = [] (const std::pair<unsigned, int> &a_pair) -> bool { return is_odd (a_pair.second); };

  std::map<unsigned, int> int_map{ { 1, 9 }, { 2, 8 }, { 3, 7 }, { 4, 6 }, { 5, 5 },
                                   { 6, 4 }, { 7, 3 }, { 8, 2 }, { 9, 1 } };
  num_odd = std::count_if (int_map.begin (), int_map.end (), is_odd_value);
  cout << format ("num_odd = {} (std::map)\n", num_odd);

  std::unordered_map<unsigned, int> int_unord_map{ { 1, 9 }, { 2, 8 }, { 3, 7 }, { 4, 6 }, { 5, 5 },
                                                   { 6, 4 }, { 7, 3 }, { 8, 2 }, { 9, 1 } };

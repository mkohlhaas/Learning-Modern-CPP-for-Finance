...
void
numerical_algorithms () // Top calling function
{
  iota_examples ();
...
accumulate_and_inner_product_examples ()
{
  ...
  // If you are curious to see what happens, you can uncomment out the next
  // three lines.

  // cout << "Putting int 0 as 1st term in accumulate sum of `double` types:\n";
  // auto mystery_sum = std::accumulate(w.begin(), w.end(), 0);		//
  // Possibly 2 cout << format("Mystery sum = {}\n", mystery_sum);
  //  Note:  You may get a compiler *warning* (a good thing), but the code will
  //  still run with bogus results.

  // Can also use std::accumulate for products (or with other arithmetic
  // operations):
  ...
  double sum_diffs
      = std::inner_product (w.begin (), w.end (), y.begin (), 0.0, std::plus<double> (), std::minus<double> ()); // -24
...
adj_diff_examples ()
{
  ...
  cout << "\nAdjacent differences with 1st element removed from front of deque "
          "(diff values only):\n";
  for (int diff : adj_diffs)
    {
      cout << diff << " ";
  ...
  cout << "\nAdjacent sums using std::adjacent_difference algo with "
          "std::plus<double>, 1st element removed from front of deque (diff "
          "values only):\n";
...
void
fold_left_examples () // See the NOTE in the chapter following
                      // std::partial_sum.
{
  using std::cout, std::format, std::vector;
  cout << "\n***std::accumulate functionality in ranges: "
          "std::ranges::fold_left_example()***\n\n";
  ...
  // int sum_of_ints = std::accumulate(v.begin(), v.end(), 0); // 621 double
  // sum_of_reals = std::accumulate(w.begin(), w.end(), 0.0);	// 0
  ...
  // double prod_of_reals = std::accumulate(w.begin(), w.end(), 1.0,
...
logs_of_adj_diff_divisions ()
{
  ...
  vector<double> prices{ 25.5, 28.0, 30.5, 31.0, 27.5, 31.0, 29.5, 28.5, 37.5, 33.5, 25.5,
                         31.5, 26.5, 29.5, 32.5, 34.5, 28.5, 35.5, 28.5, 29.0, 32.0, 23.5,
                         27.5, 33.5, 28.0, 28.0, 32.5, 31.5, 29.0, 33.0, 32.5, 29.5, 34.5 };
  ...
  // Note: The following will not compile.  Instead, wrap in a lambda as shown
  // just above. Explanation is given in the chapter:

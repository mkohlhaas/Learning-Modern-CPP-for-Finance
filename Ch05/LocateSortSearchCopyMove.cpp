...
void
locate_sort_search_copy_move () // Top calling function
{
...
copy_and_move ()
{
  ...
  // a is cleared, then makes a copy of v by assignment, so a is now {1, 2, 3,
  // 4, 5}
  a = v;
  ...
  /*
  std::deque<int> dv_copy = v;				// v is a vector,
  dv_copy is a deque std::list<int> list_v_copy = v;
  // v is a vector, list_u_copy is a list std::deque<int> dv_move =
  std::move(v);		// v is a vector, dv_move is a deque std::list<int>
  list_u_move = std::move(u);	// u is a vector, list_u_move is a dv_move
  */
  ...
  // Suppose we have a vector of Fraction objects that we want to copy to a
  // deque:

...
// #include <boost/accumulators/statistics.hpp>	// Contrary to "The Boost C++
// Libraries"
...
intro_rolling_windows ()
{
  ...
  bacc::accumulator_set<double, bacc::stats<bacc::tag::rolling_mean, bacc::tag::rolling_variance> > roll_acc{
    bacc::tag::rolling_window::window_size = 5
  };
  ...
  cout << bacc::extract::rolling_mean (roll_acc) << ", " << bacc::extract::rolling_variance (roll_acc) << "\n\n";
  ...
  // Finally, we get the rolling mean and sample variance of the last five
  // values:
  cout << bacc::extract::rolling_mean (roll_acc) << ", " << bacc::extract::rolling_variance (roll_acc) << "\n\n";
}

// n = num of std dev's, obs = num observations, win_size = num obs in rolling
// window
void
bollinger_bands (double n, unsigned obs, unsigned win_size)
{
  cout << "*** bollinger_bands(.) ***\n";

  using std::vector, Eigen::MatrixXd;

  vector<double> prices{ 100.00, 103.49, 102.82, 106.86, 104.91, 107.38, 107.46, 111.01, 112.01,
                         114.11, 116.91, 121.74, 120.04, 120.24, 120.12, 120.61, 121.31, 119.25,
                         118.11, 120.36, 117.36, 119.12, 119.36, 123.54, 123.42 };

  bacc::accumulator_set<double, bacc::stats<bacc::tag::rolling_mean, bacc::tag::rolling_variance> > prices_acc (
      bacc::tag::rolling_window::window_size = win_size);
...
void
ma_cross (unsigned fast_ma_win, unsigned slow_ma_win, unsigned obs)
{
  cout << "*** ma_cross(.) ***" << "\n";
  ...
  bacc::accumulator_set<double, bacc::stats<bacc::tag::rolling_mean> > fast_ma_acc (
      bacc::tag::rolling_window::window_size = fast_ma_win);

  bacc::accumulator_set<double, bacc::stats<bacc::tag::rolling_mean> > slow_ma_acc (
      bacc::tag::rolling_window::window_size = slow_ma_win);

...
BinomialLatticePricer::BinomialLatticePricer (OptionInfo opt, double vol, double int_rate, int time_steps,
                                              double div_rate)
...
double
BinomialLatticePricer::calc_price (double spot, OptType opt_type)
{
...
void
BinomialLatticePricer::project_underlying_prices_ (double spot)
{
...
double
BinomialLatticePricer::calculate_node_payoffs_ (OptType opt_type)
{
  // Set the terminal nodes with payoffs at expiration: j = time_points_ - 1
  for (int i = 0; i <= time_points_ - 1; ++i)
    {
      grid_[i][time_points_ - 1].payoff = opt_.option_payoff (grid_[i][time_points_ - 1].underlying);
    }
...
double
BinomialLatticePricer::disc_expected_val_ (int i, int j) const
{
  return disc_fctr_ * (p_ * grid_[i][j + 1].payoff + (1.0 - p_) * grid_[i + 1][j + 1].payoff);
}

void
BinomialLatticePricer::american_payoffs_ ()
{
  ...
  for (int j = time_points_ - 2; j >= 0; --j)
    {
      for (int i = 0; i <= j; ++i)
        {
          grid_[i][j].payoff = std::max (disc_expected_val_ (i, j), opt_.option_payoff (grid_[i][j].underlying));
        }
    }
}

void
BinomialLatticePricer::european_payoffs_ ()
{
  // Start from penultimate column prior to expiration: j = time_points_ - 2
  for (int j = time_points_ - 2; j >= 0; --j)
    {
...
void
BinomialLatticePricer::display_lattice_nodes () const
{
  using std::cout;

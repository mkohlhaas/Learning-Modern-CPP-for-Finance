...
template <typename T = double> class rand_non_central_t_dist
{
public:
...
void
statistical_distributions () // Top level calling function
{
...
density_and_distribution_fcns ()
{
  ...
  // See
  // http://www.boost.org/doc/libs/1_82_0/libs/math/doc/html/math_toolkit/stat_tut/overview/objects.html

  // Construct a standard normal distribution
  // (mean 0 and variance 1 are default ctor args):
  normal std_normal{};

  // Construct a non-standard normal distribution with mean 0.08 and variance
  // 0.25:
  ...
  // double very_high_quantile = quantile(stu_t_100, 1.0 - 1e-100);
  // // Will crash program
...
drawdown_revisited ()
{
  ...
  vector<double> max_drawdowns{
    1745348.76, 1753314.05, 1811060.31, 1819025.50, 1828980.34, 1864824.72, 1872790.60, 1945470.30, 1951446.10,
    1954544.56, 1958408.76, 1983258.00, 2059969.17, 2081873.15, 2161523.09, 2176458.22, 2185418.74, 2192386.49,
    2196369.22, 2228343.52, 2259050.25, 2275024.54, 2285932.00, 2320940.62, 2320940.63, 2346711.28, 2378568.62,
    2392511.81, 2407445.06, 2407446.45, 2421385.24, 2473268.96, 2496055.11, 2511104.90, 2514048.68, 2541852.33,
    2592631.65, 2609671.92, 2612498.83, 2635514.21, 2638433.09, 2690205.91, 2696131.21, 2698284.17, 2703144.29,
    2715096.71, 2726999.31, 2731168.85, 2762008.53, 2775829.66, 2778883.83, 2794744.74, 2800719.66, 2803706.67,
    2862401.25, 2874398.27, 2892317.42, 2956156.44, 2958145.23, 2996863.24, 3017838.81, 3082486.39, 3085470.21,
    3125295.92, 3177021.15, 3215855.22, 3293628.46, 3318451.61, 3326412.50, 3398051.61, 3423107.39, 3431069.86,
    3461772.68, 3498659.10, 3511675.10, 3517694.65, 3526652.87, 3534618.15, 3538483.24, 3617251.60, 3876004.37,
    3881100.39, 3982648.04, 3983648.65, 4003448.50, 4052305.27, 4065245.92, 4081058.35, 4132948.45, 4146771.07,
    4146966.17, 4184716.71, 4295229.34, 4417633.14, 4443583.81, 4744315.84, 4761240.15, 4777120.20, 5317756.56,
    5687134.51
  };
  ...
  // we can use the Boost normal and quantile functions for a Standard Normal
  // distribution:
  ...
  cout << "Std Norm 95%-tile ~= " << z_95
       << ",\nUsing Boost stat dist: Max DD upper 95% Conf Level = " << upper_conf_intvl_norm_95 << "\n\n";

  cout << "Std Norm 97.5%-tile ~= " << z_97_5
       << ",\nUsing Boost stat dist: Max DD upper 97.5% Conf Level = " << upper_conf_intvl_norm_97_5 << "\n\n";

  cout << "Std Norm 95%-tile ~= " << z_99
       << ",\nUsing Boost stat dist: Max DD upper 95% Conf Level = " << upper_conf_intvl_norm_99 << "\n\n";
}
...
non_central_t ()
{
  ...
  auto nc_t_gen = [&mt, &st] () { return st (mt); };

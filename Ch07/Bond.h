...
{
public:
  Bond (const std::string &bond_id, const ChronoDate &dated_date, const ChronoDate &first_coupon_date,
  ...
  // These accessors are for checking examples -- not included in the example in
  // the book:
  inline std::vector<ChronoDate>
  due_dates () const
  {
    return due_dates_;
  }
...
private:
  ...
  void calculate_pmt_schedule_ (const ChronoDate &first_coupon_date, const ChronoDate &penultimate_coupon_date,
                                const int months_in_regular_coupon_period, const double regular_coupon_payment);

  void amend_initial_irregular_dates_and_pmts_ (const ChronoDate &dated_date, const ChronoDate &first_coupon_date,

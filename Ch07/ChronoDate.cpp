...
// Properties...
unsigned
ChronoDate::days_in_month () const
{
...
bool
ChronoDate::is_end_of_month () const
{
  return date_ == date_.year () / date_.month () / std::chrono::last;
}

bool
ChronoDate::is_leap_year () const
{
...
// Operators...
int
ChronoDate::operator- (const ChronoDate &rhs) const
{
...
{
  using namespace std::chrono;
  // Note that adding days is handled differently, per Howard Hinnant's Stack
  // Overflow comments. See
...
std::ostream &
operator<< (std::ostream &os, const ChronoDate &rhs)
{
  os << rhs.ymd ();
  return os;
}
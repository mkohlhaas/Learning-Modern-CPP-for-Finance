...
date_construction ()
{
  ...
  std::chrono::year_month_day ymd{ std::chrono::year{ 2022 }, std::chrono::month{ 11 }, std::chrono::day{ 14 } };
  ...
  std::chrono::year_month_day ymd_slash = std::chrono::year{ 2022 } / std::chrono::month{ 11 } / std::chrono::day{ 15 };
...
date_accessor_functions ()
{
  ...
  unsigned the_month = static_cast<unsigned> (ymd.month ()); // 11
                                                             // (unsigned)
...
leap_years_and_last_day_of_month ()
{
  ...
  // Can check if a given date falls on eom using a function (see implementation
  // below):
  cout << "Last day of the month in which " << ymd << " falls (fcn call): " << last_day_of_the_month (ymd) << "\n\n";
...
unsigned
last_day_of_the_month (std::chrono::year_month_day &ymd)
{
  using namespace std::chrono;
...
bool
is_weekend (const std::chrono::year_month_day &ymd)
{
  using namespace std::chrono;

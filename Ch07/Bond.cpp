...
Bond::Bond (const std::string &bond_id, const ChronoDate &dated_date, const ChronoDate &first_coupon_date,
            const ChronoDate &penultimate_coupon_date, const ChronoDate &maturity_date, int coupon_frequency,
            double coupon_rate, double face_value)
    : bond_id_{ bond_id }
{
  ...
  // coupon_frequency is defined in the bond contract, and often stored in a
  // bond database. However, it is easier to use the length of the regular
  // coupon period: (1) Number of months in coupon period:
  ...
  calculate_pmt_schedule_ (first_coupon_date, penultimate_coupon_date, months_in_regular_coupon_period,
                           regular_coupon_payment);

  amend_initial_irregular_dates_and_pmts_ (dated_date, first_coupon_date, months_in_regular_coupon_period,
                                           regular_coupon_payment);

  amend_final_irregular_dates_and_pmts_ (penultimate_coupon_date, maturity_date, months_in_regular_coupon_period,
                                         regular_coupon_payment, face_value);
...
void
Bond::calculate_pmt_schedule_ (const ChronoDate &first_coupon_date, const ChronoDate &penultimate_coupon_date,
                               const int months_in_regular_coupon_period, const double regular_coupon_payment)
{
  // Bonds have many variations. Since this is not production code we simplify
  // the code by assuming the coupon day is less than 29; this avoids end of
  // month calculations. (3) Generate vectors containing due dates, payment
  // dates, and regular coupon payment amounts:
  for (ChronoDate regular_due_date{ first_coupon_date }; regular_due_date <= penultimate_coupon_date;
...
void
Bond::amend_initial_irregular_dates_and_pmts_ (const ChronoDate &dated_date, const ChronoDate &first_coupon_date,
  ...
  if (first_prior != dated_date)    // => irregular coupon
    {
      if (first_prior < dated_date) // => short coupon period
      ...
      else // dated_date < first_prior => long coupon period
        {
          ...
          // Calculate the second_prior, the last regular date before the
          // first_prior:
          ChronoDate second_prior{ first_prior };
          second_prior.add_months (-months_in_regular_coupon_period);
          double coupon_fraction
              = static_cast<double> (first_prior - dated_date) / static_cast<double> (first_prior - second_prior);
...
void
Bond::amend_final_irregular_dates_and_pmts_ (const ChronoDate &penultimate_coupon_date, const ChronoDate &maturity_date,
                                             const int    months_in_regular_coupon_period,
  ...
  if (maturity_regular_date != maturity_date)                         // => irregular coupon period	   (8)
    {
      if (maturity_date < maturity_regular_date)                      // => short coupon period
        {
          double coupon_fraction = static_cast<double> (maturity_date - penultimate_coupon_date)
      ...
      else // maturity_regular_date < maturity_date => long coupon period
        {
          ...
          double extra_coupon_fraction = static_cast<double> (maturity_date - maturity_regular_date)
...
double
Bond::discounted_value (const ChronoDate &bond_settle_date, const YieldCurve &yield_curve)
{

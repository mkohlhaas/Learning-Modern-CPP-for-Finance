...
ChronoDate
YieldCurve::settle_date () const
{
  return settle_;
}

Act365
YieldCurve::act_365 () const
{
  return act_365_;
}

double
YieldCurve::discount_factor (const ChronoDate &date_01, const ChronoDate &date_02) const
{
  using std::exp;

  if (date_02 < date_01)
    {
      throw std::invalid_argument{ "YieldCurve::discount_factor(.) invalid inequality: d2 < d1" };
    }

  if (date_01 < settle_date () || date_02 < settle_date ())
    {
      throw std::invalid_argument{ "YieldCurve::discount_factor(.): dates must "
                                   "fall on or after settle date" };
    }
    ...
    : YieldCurve{ settle_date }
{
  ...
  if (maturity_dates.size () != unit_prices.size ())
    {
      throw std::invalid_argument{ "LinearInterpYieldCurve: maturity_dates and "
                                   "spot_discount_factors different lengths" };
    }
...
double
LinearInterpYieldCurve::yield_curve_ (double t) const
{

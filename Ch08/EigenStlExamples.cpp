...
stl_and_eigen ()
{
  ...
  //	std::generate(u.data()), u.data() + u.size(), [&mt, &tdist]() {return
  // tdist(mt);});

  cout << "t-dist variates in VectorXd:\n" << u << "\n\n";

  // Non-modifying std::max_element algo:
  auto max_u = std::max_element (u.begin (),
                                 u.end ()); // max_element(.) returns an iterator
  ...
  cout << "Vector sum of u (Eigen::VectorXd) and v (std::vector) using "
          "std::transform:\n";
  for (double x : w)
    {
      cout << x << " ";
...
rowwise_and_colwise ()
{
  ...
  // Easy example:
  MatrixXd vals_01{ { 9.0, 8.0, 7.0 }, { 3.0, 2.0, 1.0 }, { 9.5, 8.5, 7.5 }, { 3.5, 2.5, 1.5 } };
...
log_rtns_rowwise_colwise ()
{
  ...
  MatrixXd prices_to_returns{ { 25.5, 8.0, 70.5 }, { 31.0, 7.5, 71.0 }, { 29.5, 8.5, 77.5 }, { 33.5, 5.5, 71.5 },
                              { 26.5, 9.5, 72.5 }, { 34.5, 8.5, 75.5 }, { 28.5, 9.0, 72.0 }, { 23.5, 7.5, 73.5 },
                              { 28.0, 8.0, 72.5 }, { 31.5, 9.0, 73.0 }, { 32.5, 9.5, 74.5 } };
  ...
  cout << "Adjacent differences of logs of prices (but 1st row still contains "
          "prices): "
       << "\n"
       << prices_to_returns << "\n\n";
  ...
  for (const auto &row : returns_mtx.rowwise ())
    {
      for (double element : row)
        {
          std::cout << std::right << std::setw (7) << std::fixed << std::setprecision (2) << element * 100 << "%\t";
        }
      std::cout << "\n";
    }
  cout << "\n\n";
  ...
  for (auto j = 0; j < monthly_returns.size (); ++j)
    {
      cout << std::right << std::setw (7) << std::fixed << std::setprecision (2) << monthly_returns (j) * 100 << "%\n";
    }
...
eigen_mtx_unary_expr ()
{
  ...
  cout << "Now take square root of each element in the updated vals mtx with "
          "unaryExpr(.): "
       << "\n";
